...
function UserAvatar({
  ...
  if (avatarUrl) {
    return (
      ...
      <img
        ...
        className={`${sizeClasses} rounded-full ${size === "sm" ? "border-2 border-violet-500" : ""}`}
      />
    );
  }
  ...
  return (
    <div
      className={`${sizeClasses} rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center text-white font-medium`}
    >
      {initial}
    </div>
  );
