...
export default function Home(): JSX.Element {
  ...
  const shareOnLinkedIn = (title: string) => {
    trackShareTitle("linkedin", title);
...
Also identify exactly 4 must-have SEO keywords for this topic that writers should include in their articles.
...
Also identify exactly 4 relevant keywords for this topic that writers should consider including in their articles.
...
KEYWORDS: keyword1, keyword2, keyword3, keyword4`;
    ...
    try {
      ...
      // Parse the response to extract titles and keywords
      const keywordsMatch = responseContent.match(/KEYWORDS:\s*(.+)/i);
      if (keywordsMatch) {
      ...
      // Extract just the titles portion
      const titlesMatch = responseContent.match(
        /TITLES:\s*([\s\S]*?)(?=\n\s*KEYWORDS:|$)/i,
      );
      if (titlesMatch) {
        setGeneratedTitles(titlesMatch[1].trim());
      } else {
        // Fallback: if format doesn't match, use original content without keywords line
                  ...
                  {seoKeywords.length > 0 && (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs sm:text-sm font-semibold text-zinc-800 dark:text-zinc-300">
                          {t("home.seoKeywords")}
                        </span>
                      </div>
